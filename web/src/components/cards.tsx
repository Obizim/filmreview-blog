...
export const Card = ({posts}: postProps) => {
  return (
   <ul className="py-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {posts.map((post) => (
      <Link href={`/post/${post.slug}`} key={post._id} className="py-6 px-4 border border-gray-600 rounded-lg">
        <li>
          <div className="flex font-light text-sm">
            ...
            {post.categories.map((category, i) => (
              <p key={i} className="lowercase">{category}</p>
            ))}
          </div>
          <h2 className="font-bold text-xl mt-2 capitalize line-clamp-3">{post.title}</h2>
          <div className="leading-relaxed pt-4">
